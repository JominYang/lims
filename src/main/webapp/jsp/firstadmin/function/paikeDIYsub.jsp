...
<html>
  ...
  <% 
	...
	String kebiaoname =(String)request.getAttribute("cuname");
	//获得开始时间
	String sdate = (String)request.getAttribute("dateStart");
	
		String edate = (String)request.getAttribute("dateEnd");
	String labname = (String)request.getAttribute("Labroom");
  	...
  	<tr>
  		...
  		<td align="left"><%=edate %></td>
			...
			<tr>
				<td class="blank"></td>
				<%if(curentpage!=1) {%>
				<td class="title" colspan="2"><a href="<%=request.getContextPath() %>/PaikeSubServlet?sub=paikeByhand&weekpage=<%=(curentpage-1)%>&cuname=<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode(kebiaoname.toString(),"UTF-8")) %>&Labroom=<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode(labname.toString(),"UTF-8"))%>&dateStart=<%=sdate %>&dateEnd=<%=edate%>%>&dateStart=<%=sdate %>&dateEnd=<%=edate%>">上一周</a></td>
				<%} else{%>
				...
				<td class="title" colspan="2"><a href="<%=request.getContextPath() %>/PaikeSubServlet?sub=paikeByhand&weekpage=<%=(curentpage+1)%>&cuname=<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode(kebiaoname.toString(),"UTF-8"))%>&dateStart=<%=sdate %>&dateEnd=<%=edate%> %>&Labroom=<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode(labname.toString(),"UTF-8"))%>&dateStart=<%=sdate %>&dateEnd=<%=edate%>">下一周</a></td>
				<%} else{%>
