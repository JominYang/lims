...
<html>
  ...
  <script type="text/javascript" charset="UTF-8">
  	...
  	$(function(){ 
        ...
        url:sy.bp()+'/TeaFuctionServlet?method=expSearch',
        editable:true,
        valueField:'id',
        textFiled:'name'
      });
  		...
  		datagrid = $('#datagrid').datagrid({
  				url: sy.bp()+'/CourseSearch?method=searchAllbyTea',
				title: '实验课程信息表',
				iconCls:'icon-save',
				fit:true,
				pagination:true,
				...
				columns:[[
					...
					{field:'yixuan',title:'上课人数',	width : 100},
					{field:'detail',title:'登录成绩',	width : 100,
						formatter :function(value,rowData,rowIndex) {
							return '<a href="javascript:void(0);" onclick="teaSub('+rowIndex+');">登录成绩</a>';
						}
					}
				]],
  		});
  	});
  	function teaSub(index){
  		var rows = datagrid.datagrid('getRows');
  		var row = rows[index];
  		var kechengId = row.id;
  		window.location=sy.bp()+"/DCJServlet?kechengId="+kechengId;
  	}
